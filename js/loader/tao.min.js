...
)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.elements)?helper.call(depth0,{hash:{},data:data}):helper)||0===stack1)&&(buffer+=stack1),buffer+='<span class="viewable-hiddenbox-toggle" tabindex="0"><span class="icon-preview"></span><span class="icon-eye-slash" style="display: none;"></span></span></span>'})});var theAnswer=require("theAnswer");console.log("a =",theAnswer),module.exports={theAnswer:theAnswer},define("npm-imports",[],function(){}),define("theAnswer",function(){}),define("controller/login",["jquery","lodash","i18n","module","layout/loading-bar","layout/version-warning","ui/login/login","theAnswer"],function($
,_,__,module,loadingBar,versionWarning,loginComponent,theAnswer){"use strict";var _defaults={disableAutocomplete:!1,enablePasswordReveal:!1,message:{error:""}};return console.warn("a =",theAnswer),{start:function(){var conf=_.defaults({},module.config(),_defaults);loginComponent($("#login-box-inner-container"),conf).on("init",function(){loadingBar.start()}).after("render",function(){versionWarning.init(),loadingBar.stop()}).on("submit.login",function(){loadingBar.start()})}}}),define("form/post-render-props",["jquery","i18n","ui/feedback"],function($,__,feedback){"use strict";function _toggleModeBtn(newMode){var $modeToggle=$(".property-mode")
;"disabled"===newMode?$modeToggle.addClass("disabled"):$modeToggle.removeClass("disabled")}function _upgradeButtons($container,type){"radio"!==type&&"checkbox"!==type&&(type="radio");var selector=".form_checklst",notSelector="";"radio"===type&&(selector=".form_radlst",notSelector=".form_checklst, "),$container.find(selector).not(notSelector+".property-"+type+"-list").each(function(){var $radioList=$(this);$radioList.addClass("property-"+type+"-list"),$radioList.parent().addClass("property-"+type+"-list-box"),$radioList.each(function(){var $block=$(this),$inputs=$block.find("input");$inputs.length<=2&&$block.find("br").remove(),$inputs.each(function(){
var $input=$(this),$label=$block.find('label[for="'+this.id+'"]'),$icon=$("<span>",{class:"icon-"+type});$label.prepend($icon),$label.prepend($input)})})})}function getPropertyContainer(){var $propertyContainer=$(".content-block .property-container");return $propertyContainer.length||($propertyContainer=$("<div>",{class:"property-container"}),$(".content-block .form-group").first().before($propertyContainer)),$propertyContainer}function _hideProperties($container){$(".property",$container).each(function(){for(var $currentTarget=$(this);!_.isEqual($currentTarget.parent()[0],$container[0]);)$currentTarget=$currentTarget.parent();$currentTarget.hide()}),
_toggleModeBtn("disabled")}function _showProperties($container){$(".property",$container).each(function(){for(var $currentTarget=$(this);!_.isEqual($currentTarget.parent()[0],$container[0]);)$currentTarget=$currentTarget.parent();$currentTarget.show()});var elt=$('[class*="property-type"]',$container).parent("div").next("div");new RegExp(["list","tree"].join("$|").concat("$")).test($('[class*="property-type"]',$container).val())?"none"===elt.css("display")&&(elt.show(),elt.find("select").removeAttr("disabled")):"none"!==elt.css("display")&&(elt.css("display","none"),elt.find("select").prop("disabled","disabled")),_toggleModeBtn("enabled")}
function _hideIndexes($container){$(".index",$container).each(function(){for(var $currentTarget=$(this);!_.isEqual($currentTarget.parent()[0],$container[0]);)$currentTarget=$currentTarget.parent();$currentTarget.hide()}),$(".index-remover",$container).each(function(){$(this).parent().hide()})}function _showIndexes($container){$(".index",$container).each(function(){for(var $currentTarget=$(this);!_.isEqual($currentTarget.parent()[0],$container[0]);)$currentTarget=$currentTarget.parent();$currentTarget.show()}),$(".index-remover",$container).each(function(){$(this).parent().show()})}return{init:function($properties){var $container=$(
".content-block .xhtml_form:first form");$container.length&&(function($container){var $copyField,isInstanceForm=document.getElementById("tao.forms.instance"),$idField=$container.find("#id"),value=$idField.val(),$label=$("<span>",{class:"form_desc",text:__("Resource Identifier")}),$button=$("<span>",{class:"icon-clipboard clipboard-command",title:__("Copy to clipboard")}).on("click",function(){var success;try{$copyField.select(),success=document.execCommand("copy"),$copyField.blur(),success?feedback().success(__("Resource Identifier has been copied to the clipboard")):feedback().error(__("Resource Identifier could not be copied to the clipboard"))}catch(err){
feedback().error(__("Your browser does not support copying to the clipboard"))}}),$fieldBox=$("<span>",{class:"uri-container"});$(".uri-container").length||isInstanceForm&&$idField.length&&($copyField=$idField.clone().val("").attr({readonly:!0,type:"text"}),$idField.remove(),$fieldBox.append([$copyField,$button]),$container.find("div").first().after($("<div>").append([$label,$fieldBox])),$fieldBox.height($copyField.outerHeight()),$copyField.addClass("final").val(value))}($container),$properties&&$properties.length||($properties=$container.children('div[id*="property_"]').not(".property-block")),$properties.length&&(function($properties){
